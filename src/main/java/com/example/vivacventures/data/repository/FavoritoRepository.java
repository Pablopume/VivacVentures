...
//    void deleteByUserAndVivacPlace(UserEntity user, VivacPlaceEntity vivacPlace);
    @Transactional
    @Modifying
    void deleteByListaAndVivacPlace(ListaEntity user, VivacPlaceEntity vivacPlace);

    @Query("SELECT f FROM FavoritoEntity f LEFT JOIN FETCH f.vivacPlace vp LEFT JOIN FETCH vp.valorations WHERE f.lista = :lista")
    List<FavoritoEntity> findByUserFetch(ListaEntity lista);
